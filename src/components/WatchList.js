...
const WatchList = () => {
  ...
  console.log(user);
          ...
          useEffect(() => {
            ...
            console.log(runner);
            // console.log(runner[2].genres.map(item=>item[0].name));
            // console.log(runner.genres.map(item=>item.name));
  ...
  return (
    <Main className="mt-[100px]">
      ...
      <div className="      mt-[50px] flex-col  flex  items-center   justify-start absolute w-[100%] h-[100%]">
        { 
        runner
        &&
        runner.map((item) => (
              ...
              <Const className="w-[100%] h-[100%] z-[-1]  hover:scale-[1.01]   transition-all object-cover relative rounded-[10px]">
                <div className="rounded-[10px]">
                  <h1 className=" absolute pt-[12px] pl-[45px] text-[1.45rem] tracking-[1.5px]">
                    {item.title||item.name||item.original_name||item.original_title||item.original_name}
                  </h1>
                  <h1 className=" absolute pt-[50px] pl-[50px] text-[0.95rem] text-gray-400 tracking-[1.5px] ">
                    Action,Adventure,Sci-Fi
                      
                    {item.first_air_date && item.first_air_date.slice(0, 4)||item.release_date&&item.release_date.slice(0,4)}
                  </h1>
                  ...
                  <button className="hover:text-gray-300 hover:bg-gray-700 hover:transition-all hover:scale-[1.07] hover:border-gray-600 flex justify-center items-center  text-center absolute py-[6px] px-[10px] ml-[185px] text-gray-600 mt-[170px]  text-[1.05rem] border-[2.8px]  bg-[#0f172a] rounded-[6px] border-gray-800 ">
